...
# Velocidades reducidas para simular cautela y tension
...
func _ready():
	# --- Agregado: se agrega al grupo "player" para que la Entity lo detecte ---
	add_to_group("player")

	# 1. Graficos del jugador
	...
	camera.position_smoothing_speed = 5.0 # Mas bajo = la camara tarda mas en seguirte (desorientacion)
	camera.zoom = Vector2(1.5, 1.5) # Zoom muy cerrado para generar claustrofobia
	add_child(camera)
	camera.make_current()

func _physics_process(_delta):
	# --- Agregado: si ya moriste, quedas congelado ---
	if GameState.is_dead:
		velocity = velocity.lerp(Vector2.ZERO, friccion)
		move_and_slide()
		return

	# --- Agregado: mecanica Hold Breath - congela el movimiento mientras se aguanta la respiracion ---
	...
	# Logica de inercia y peso (reemplaza el arranque instantaneo)
